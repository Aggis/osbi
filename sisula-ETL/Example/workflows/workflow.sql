...
EXEC sp_add_jobstep
    ...
    @command = '
            $files = @(Get-ChildItem FileSystem::"C:\dev\osbi\sisula-ETL\Example\data\incoming" | Where-Object {$_.Name -match "[0-9]{5}_Collisions_.*\.csv"})
            If ($files.length -eq 0) {
              Throw "No matching files were found in C:\dev\osbi\sisula-ETL\Example\data\incoming"
            } Else {
                ForEach ($file in $files) {
                    $fullFilename = $file.FullName
                    Move-Item $fullFilename C:\dev\osbi\sisula-ETL\Example\data\work -force
                    Write-Output "Moved file: $fullFilename to C:\dev\osbi\sisula-ETL\Example\data\work"
                }
            }
        ',
    @on_success_action = 3,
...
EXEC sp_add_jobstep
    @subsystem = 'PowerShell',
    @command = '
            $files = @(Get-ChildItem -Recurse FileSystem::"C:\dev\osbi\sisula-ETL\Example\data\work" | Where-Object {$_.Name -match "[0-9]{5}_Collisions_.*\.csv"})
            If ($files.length -eq 0) {
              Throw "No matching files were found in C:\dev\osbi\sisula-ETL\Example\data\work"
            } Else {
                ForEach ($file in $files) {
                    $fullFilename = $file.FullName
                    ...
                    Move-Item $fullFilename C:\dev\osbi\sisula-ETL\Example\data\archive -force
                    Write-Output "Moved file: $fullFilename to C:\dev\osbi\sisula-ETL\Example\data\archive"
                }
            }
        ',
    @database_name = 'Stage',
...
DECLARE @xml XML = N'<workflow name="NYPD_Vehicle_Workflow">
	...
	<variable name="incomingPath" value="C:\dev\osbi\sisula-ETL\Example\data\incoming"/>
	<variable name="workPath" value="C:\dev\osbi\sisula-ETL\Example\data\work"/>
	<variable name="archivePath" value="C:\dev\osbi\sisula-ETL\Example\data\archive"/>
